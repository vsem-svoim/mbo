...
export function OrderRow({ level, side, maxTotal }: OrderRowProps) {
  ...
  const badge = level.isAggressive ? (
    <span className="text-[9px] font-bold px-1 py-0.5 rounded bg-slate-600 text-white ml-1">
      AGG
    </span>
  ) : (
    <span className="text-[9px] font-bold px-1 py-0.5 rounded bg-slate-700 text-slate-400 ml-1">
      PAS
    </span>
  );
  ...
  return (
    <div className="relative grid grid-cols-[1fr_1fr_0.8fr] gap-2 px-3 py-1 hover:bg-white/5">
      <div className={`font-semibold ${side === "ask" ? "text-trade-sellText" : "text-trade-buyText"}`}>
      ...
      <div
        className={`absolute top-0 right-0 h-full opacity-30 ${
          side === "ask" ? "bg-trade-sell" : "bg-trade-buy"
        }`}
